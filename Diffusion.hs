...
--helperTeileThis n xs = [take 4 xs] ++[(take 4 . drop 4)xs] ++ [[x | x <- take 8 (drop (8*i) (drop 4 xs))] | i <- [0..n-1]]
...
-- für eine ganzes Bild
--pointBild = foldr (++) [] [pointGenerate n | n <- [0..29]] diffOP point  = [ mv n m point | (mv, (n,m)) <- zip (cycle movingList) pointBild, f mv n m point ]
--  where 
--  f mv n m p | mv n m p == p    = False
--             | otherwise         = True
--
--------------------------------------------------
...
getField mySC | endOfField $ currentpoint mySC  = mySC
              | otherwise 
  ...
  where
    ...
    savePoint xs = (currentpoint mySC, color mySC) : xs
--    visitBefore a = not $ any (\ (p,c) -> p == a) $ (take 1 . reverse) $ sellevel mySC
    visitBefore a = a == currentpoint mySC 
    goFurther moveto p 
...
northk k (x,y)  | y-k >= 0          = (x,y-k)
                | otherwise         = (x,y)
westk  k (x,y)  | x+k <= fst dim    = (x+k,y)
                | otherwise         = (x,y)
southk k (x,y)  | y+k <= snd dim    = (x,y+k)
                | otherwise         = (x,y)
eastk  k (x,y)  | x-k >= 0          = (x-k,y)
                | otherwise         = (x,y)
...
nw n m p 
  ...
  | otherwise                        = p
sw n m p 
  | checkFormv southk westk n m p  = ita southk westk n m p
  | otherwise                        = p
se n m p 
